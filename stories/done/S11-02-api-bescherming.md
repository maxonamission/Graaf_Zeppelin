...
### Frontend (XSS/CSP)
- [ ] `login.html`: `data.detail` via `textContent` i.p.v. `innerHTML`
- [ ] `register.html`: idem
- [ ] `graph_viewer.html`: domein-namen en factor-ID's in onclick via `data-*` attributen + event delegation i.p.v. string-interpolatie
- [ ] `reasoning.html`: `escapeAttr()` vervangen door `JSON.stringify()` voor attribuutwaarden
- [ ] Alle `innerHTML` paden consistent via `escapeHtml()` of `textContent`
- [ ] Content Security Policy header:
  ...
  ```
- [ ] SRI-hashes op externe scripts (htmx, D3):
  ```html
